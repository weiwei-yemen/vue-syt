<template>
  <div
    ...
    v-if="pageInfo.hospitalInfo.hospital && route.name === 'registration'"
  >
    ...
    <div class="logo-route">
      ...
      <div class="m-20">
        <h3>挂号规则</h3>
        <span class="m-r-10"> 预约周期：{{ pageInfo.hospitalInfo.bookingRule.cycle }}天 </span>
        <span class="m-r-10"> 放号时间：{{ pageInfo.hospitalInfo.bookingRule.releaseTime }} </span>
        <span class="m-r-10"> 停挂时间：{{ pageInfo.hospitalInfo.bookingRule.stopTime }} </span>
        <br />
        ...
        <h3>医院预约规则</h3>
        <ol>
          <li v-for="(item, index) in pageInfo.hospitalInfo?.bookingRule?.rule" :key="index">
            {{ `${index + 1}. ${item}` }}
          </li>
        </ol>
      </div>
